...
<div class="login-box">
    ...
    <div class="login-box-body">
        ...
        <form action="{{ admin_url('auth/login') }}"
              method="post">
                ...
                @if($errors->has('username'))
                    @foreach($errors->get('username') as $message)
                        <label class="control-label"
                               for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label><br>
                    @endforeach
                @endif
            ...
            @if($locale && isset($locale['languages']))
                <div class="form-group">
                    <select class="form-control"
                            id="locale">
                        @foreach($locale['languages'] as $key => $language)
                            <option value="{{$key}}" {!! $key != $locale['current'] ?: 'selected' !!}>{{$language}}</option>
                        @endforeach
                    </select>
                </div>
            @endif
